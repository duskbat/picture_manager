    ...
    <div class="midCont">
        ...
        <div class="favouriteBar">
            <ul class="nav nav-pills">
                <li class="active"><a href="company_video_auditing" >审核中</a></li>
                <li  ><a href="company_video_audited">审核通过</a></li>
                <li ><a href="company_video_unaudited">审核未通过</a></li>
            </ul>


            <a href="company_auditing" class="edit_selection" style="display: block;font-size: 16px;right: 160px;background-color: #515151">进入图片审核页</a>
...
-->
                <li>
                    <a href="${pageContext.request.contextPath}/company_video_auditing?pn=1">首页</a>
                </li>
                <!-- 如果还有前页就访问当前页码-1的页面， -->
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/company_video_auditing?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                ...
                <c:if test="${page_Nums!=pageInfo.pageNum }">
                    <li ><a href="${pageContext.request.contextPath}/company_video_auditing?pn=${page_Nums}">${page_Nums}</a></li>
                </c:if>
                </c:forEach>
                </li>
                <!-- 如果还有后页就访问当前页码+1的页面， -->
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/company_video_auditing?pn=${pageInfo.pageNum+1}" aria-label="Next">
...
<form id="company_video_audit" method="post">
    <input type="hidden" id="userid" name="id" />
</form>
<script type="text/javascript">

    function company_video_audit(w){
        var imageId = $(w).children().attr("id");
        $("#company_video_audit").attr("action","<%=basePath%>company_video_audit_detail");
        $("#userid").attr("value",imageId);
        $("#company_video_audit").submit();
    }

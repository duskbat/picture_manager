...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            <li role="presentation" class="active"><a href="client_deal" >待公司确认</a></li>
        ...
        <table class="table table-striped table-hover table-bordered" style="margin-left: 70px;margin-top: 30px;font-size: 12px">
            <thead>
            <tr>
                <th>版权使用人</th>
                <th>图片名</th>
                ...
                <th>开始时间</th>
                <th>截止时间</th>
                <th id="paytimeth" style="display: none">付款时间</th>
                <th>操作</th>
            </tr>

            <c:forEach items="${pageInfo.list}" var="deal">
                <%--<c:if test="${deal.contractState==1 && deal.orderUserid==user.id}">--%>
                    <tr>
                        ...
                        <th id="paytimethfmt" style="display: none"><fmt:formatDate value="${deal.payTime}" type="both" dateStyle="long" /></th>

                        <th>
                            <span class="scriptionLeftSpan" style="margin-left: 12px" id="${deal.id}">
                                 <button class='btn btn-primary edit_button' type='submit' style="width: 50px;height: 36px;padding: 1px;font-size: 14px">付款</button>
                            </span>
                            <span class="scriptionLeftSpan" style="margin-left: 12px" id="${deal.id}">
                                 <button class='btn btn-danger delete_button' style="width: 50px;height: 36px;padding: 1px;font-size: 14px">删除</button>
                            </span>
                        </th>
                    </tr>
            <%--</c:if>--%>
        ...
        -->
                    <li>
                        <a href="${pageContext.request.contextPath}/client_deal?pn=1">首页</a>
                    </li>
                    <!-- 如果还有前页就访问当前页码-1的页面， -->
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/client_deal?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                    ...
                    <c:if test="${page_Nums!=pageInfo.pageNum }">
                        <li ><a href="${pageContext.request.contextPath}/client_deal?pn=${page_Nums}">${page_Nums}</a></li>
                    </c:if>
                    </c:forEach>
                    </li>
                    <!-- 如果还有后页就访问当前页码+1的页面， -->
                    <c:if test="${pageInfo.hasNextPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/client_deal?pn=${pageInfo.pageNum+1}" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                    </c:if>
                    <li><a href="${pageContext.request.contextPath}/client_deal?pn=${pageInfo.pages}">末页</a></li>
                </ul>
                <%--</nav>--%>
            </div>
        </div>

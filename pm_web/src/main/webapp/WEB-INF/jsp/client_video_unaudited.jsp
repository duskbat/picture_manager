...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            <li role="presentation" ><a href="client_video_auditing" >审核中</a></li>
            ...
            <li role="presentation" class="active"><a href="client_video_unaudited">审核未通过</a></li>
        </ul>

        <span>当前页面:</span>
        <span style="color: #0074D9">视频审核未通过</span>

        <a id="client_unauditvideo_all_check"  class="edit_selection all_check" style="display: block;font-size: 16px;right: 200px;background-color: #66afe9">全&nbsp;选&nbsp;/&nbsp;取&nbsp;消</a>
        <a id="client_unaudit_back" class="edit_selection" style="display: block;font-size: 16px;right: 90px;">退回修改</a>
        ...
        <div class="favouriteGalleryWrap">
            <div class="favouriteGallery layout3 globalDetail" style="position: relative; height: 0px;">
                <c:forEach items="${pageInfo.list}" var="picture">
                    ...
                    <span style="margin-right: 36px;color: #0074D9">
                        退回原因:<span style="margin-left: 6px">${picture.reason}</span>
                    </span>
...
-->
                <li>
                    <a href="${pageContext.request.contextPath}/client_video_unaudited?pn=1">首页</a>
                </li>
                <!-- 如果还有前页就访问当前页码-1的页面， -->
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_video_unaudited?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                ...
                <c:if test="${page_Nums!=pageInfo.pageNum }">
                    <li ><a href="${pageContext.request.contextPath}/client_video_unaudited?pn=${page_Nums}">${page_Nums}</a></li>
                </c:if>
                </c:forEach>
                </li>
                <!-- 如果还有后页就访问当前页码+1的页面， -->
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_video_unaudited?pn=${pageInfo.pageNum+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
                <li><a href="${pageContext.request.contextPath}/client_video_unaudited?pn=${pageInfo.pages}">末页</a></li>
            </ul>
            <%--</nav>--%>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#client_unaudit_back").click(function() {
...
//                        alert("失败");
        ...
        // 全选/取消全部
        $("#client_unauditvideo_all_check").click(function() {

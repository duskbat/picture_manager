...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            ...
            <li role="presentation"  ><a href="client_audited" >审核通过</a></li>
            <li role="presentation"><a href="client_audited_post" >发布成功</a></li>
            <li role="presentation" class="active"><a href="client_unaudited">审核未通过</a></li>
        </ul>

        <a href="javascript:void(0)" id="client_unaudit_all_check" class="edit_selection" style="display: block;font-size: 16px;right: 200px;background-color: #66afe9">全&nbsp;选&nbsp;/&nbsp;取&nbsp;消</a>
        <a href="javascript:void(0)" id="client_unaudit_back" class="edit_selection" style="display: block;font-size: 16px;right: 90px;">退回修改</a>
        ...
        <div class="favouriteGalleryWrap">
            <div class="favouriteGallery layout3 globalDetail" style="position: relative; height: 0px;">
                ...
                <c:forEach items="${pageInfo.list}" var="picture">
                    <%--<c:if test="${picture.stateId==8 && picture.userId==user.id}">--%>
...
-->
                <li>
                    <a href="${pageContext.request.contextPath}/client_unaudited?pn=1">首页</a>
                </li>
                <!-- 如果还有前页就访问当前页码-1的页面， -->
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_unaudited?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                ...
                <c:if test="${page_Nums!=pageInfo.pageNum }">
                    <li ><a href="${pageContext.request.contextPath}/client_unaudited?pn=${page_Nums}">${page_Nums}</a></li>
                </c:if>
                </c:forEach>
                </li>
                <!-- 如果还有后页就访问当前页码+1的页面， -->
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_unaudited?pn=${pageInfo.pageNum+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
                <li><a href="${pageContext.request.contextPath}/client_unaudited?pn=${pageInfo.pages}">末页</a></li>
            </ul>
            <%--</nav>--%>
        </div>
    </div>
...
//                        alert("失败");
        ...
        // 全选/取消全部
        $("#client_unaudit_all_check").click(function() {

...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            ...
            <li role="presentation" ><a href="company_deal_success" >公司已签字</a></li>
            <li role="presentation" ><a href="company_success_client" >公司未付款</a></li>
            <li role="presentation"  class="active"><a href="company_over_client" >公司已付款</a></li>
        </ul>
        ...
        <table class="table table-striped table-hover table-bordered" style="margin-left: 70px;margin-top: 30px">
            ...
            <c:forEach items="${price}" var="everyprice">
                    ...
                    <tr>
                        ...
                        <th>
                            <span class="scriptionLeftSpan" style="margin-left: 12px" id="${deal.id}">
                                 <button class='btn btn-primary company_pay_button' type='submit' style="width: 50px;height: 36px;padding: 1px;font-size: 14px">详情</button>
                            </span>
                        </th>
                    </tr>
            </c:forEach>
        ...
        -->
                    <li>
                        <a href="${pageContext.request.contextPath}/company_over_client?pn=1">首页</a>
                    </li>
                    <!-- 如果还有前页就访问当前页码-1的页面， -->
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/company_over_client?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                    ...
                    <c:if test="${page_Nums!=pageInfo.pageNum }">
                        <li ><a href="${pageContext.request.contextPath}/company_over_client?pn=${page_Nums}">${page_Nums}</a></li>
                    </c:if>
                    </c:forEach>
                    </li>
                    <!-- 如果还有后页就访问当前页码+1的页面， -->
                    <c:if test="${pageInfo.hasNextPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/company_over_client?pn=${pageInfo.pageNum+1}" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                    </c:if>
                    <li><a href="${pageContext.request.contextPath}/company_over_client?pn=${pageInfo.pages}">末页</a></li>
                </ul>
                <%--</nav>--%>
            </div>
        </div>
...
<%--提交详情表单--%>
<form id="company_pay_edit" method="post">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(".company_pay_button").click(
            function ()
            {
                $("#company_pay_edit").attr("action","<%=basePath%>company_pay_edit");
                $("#userid").attr("value",$(this).parent().attr("id"));
                $("#company_pay_edit").submit();
            }
        );

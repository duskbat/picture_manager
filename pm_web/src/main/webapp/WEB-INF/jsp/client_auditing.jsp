...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            <li role="presentation" class="active"><a href="client_auditing" >审核中</a></li>
        ...
        <a href="client_video_auditing" class="edit_selection" style="display: block;font-size: 16px;right: 160px;background-color: #4a86e8">视频审核页</a>
        ...
        <div class="favouriteGalleryWrap">
            <div class="favouriteGallery layout3 globalDetail" style="position: relative; height: 0px;">
                ...
                <c:forEach items="${pageInfo.list}" var="picture">
                    <%--<c:if test="${picture.stateId==6 && picture.userId==user.id}">--%>
...
-->
                <li>
                    <a href="${pageContext.request.contextPath}/client_auditing?pn=1">首页</a>
                </li>
                <!-- 如果还有前页就访问当前页码-1的页面， -->
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_auditing?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                ...
                <c:if test="${page_Nums!=pageInfo.pageNum }">
                    <li ><a href="${pageContext.request.contextPath}/client_auditing?pn=${page_Nums}">${page_Nums}</a></li>
                </c:if>
                </c:forEach>
                </li>
                <!-- 如果还有后页就访问当前页码+1的页面， -->
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_auditing?pn=${pageInfo.pageNum+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
                <li><a href="${pageContext.request.contextPath}/client_auditing?pn=${pageInfo.pages}">末页</a></li>
            </ul>
            <%--</nav>--%>
        </div>
    </div>

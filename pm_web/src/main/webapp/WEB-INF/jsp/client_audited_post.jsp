...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            ...
            <li role="presentation" class="active" ><a href="client_audited_post" >发布成功</a></li>
            <li role="presentation" ><a href="client_unaudited">审核未通过</a></li>
        </ul>

        <span>当前页面:</span>
        <span style="color: #0074D9">图片成功发布</span>
        ...
        <div class="favouriteGalleryWrap">
            <div class="favouriteGallery layout3 globalDetail" style="position: relative; height: 0px;">
                ...
                <c:forEach items="${pageInfo.list}" var="picture">
                    <%--<c:if test="${picture.stateId==3 && picture.userId==user.id}">--%>
...
-->
                <li>
                    <a href="${pageContext.request.contextPath}/client_audited_post?pn=1">首页</a>
                </li>
                <!-- 如果还有前页就访问当前页码-1的页面， -->
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_audited_post?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                ...
                <c:if test="${page_Nums!=pageInfo.pageNum }">
                    <li ><a href="${pageContext.request.contextPath}/client_audited_post?pn=${page_Nums}">${page_Nums}</a></li>
                </c:if>
                </c:forEach>
                </li>
                <!-- 如果还有后页就访问当前页码+1的页面， -->
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/client_audited_post?pn=${pageInfo.pageNum+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
                <li><a href="${pageContext.request.contextPath}/client_audited_post?pn=${pageInfo.pages}">末页</a></li>
            </ul>
            <%--</nav>--%>
        </div>
    </div>
...
<form id="clientpostForm" method="post">
...
<script type="text/javascript">
    ...
    function imageDetail(w){
        ...
        $("#clientpostForm").attr("action","<%=basePath%>index_detail");
        $("#userid").attr("value",imageId);
        $("#clientpostForm").submit();
    }

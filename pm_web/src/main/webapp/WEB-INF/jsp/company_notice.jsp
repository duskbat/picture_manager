...
<div class="userCenterWrap">
    ...
    <div class="midCont">
        ...
        <ul class="nav nav-pills">
            ...
            <a class="edit_selection add_button" style="display: block;font-size: 16px;right: 130px;background-color: #66afe9">添加公告</a>
            </form>
        ...
        -->
        <li>
        <a href="${pageContext.request.contextPath}/company_notice?pn=1">首页</a>
        </li>
        <!-- 如果还有前页就访问当前页码-1的页面， -->
        <c:if test="${pageInfo.hasPreviousPage}">
        <li>
        <a href="${pageContext.request.contextPath}/company_notice?pn=${pageInfo.pageNum-1}" aria-label="Previous">
        ...
        <c:if test="${page_Nums!=pageInfo.pageNum }">
        <li ><a href="${pageContext.request.contextPath}/company_notice?pn=${page_Nums}">${page_Nums}</a></li>
        </c:if>
        </c:forEach>
        </li>
        <!-- 如果还有后页就访问当前页码+1的页面， -->
        ...
        <li>
        <a href="${pageContext.request.contextPath}/company_notice?pn=${pageInfo.pageNum+1}" aria-label="Next">
        <span aria-hidden="true">&raquo;</span>
        </a>
        </li>
        </c:if>
        <li><a href="${pageContext.request.contextPath}/company_notice?pn=${pageInfo.pages}">末页</a></li>
        </ul>
        <%--</nav>--%>
        </div>
        </div>
...
<form id="company_notice_edit" method="post">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(".edit_button").click(
            function ()
            {
                $("#company_notice_edit").attr("action","<%=basePath%>company_notice_edit");
                $("#userid").attr("value",$(this).parent().attr("id"));
                $("#company_notice_edit").submit();
            }
        );
        ...
        $(".add_button").click(
            function ()
            {
                $("#addForm").attr("action","<%=basePath%>addNotice");
                $("#addForm").submit();
            }
        );

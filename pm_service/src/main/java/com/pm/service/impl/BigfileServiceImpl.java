...
//        Long userId = ((User)request.getSession().getAttribute("user")).getId();
        ...
        filePath = ""+ UUID.randomUUID().toString()+"."+plupload.getName().substring(plupload.getName().lastIndexOf(".") + 1); //在服务器内生成唯一文件名
        filePathshow = ""+UUID.randomUUID().toString()+"."+plupload.getName().substring(plupload.getName().lastIndexOf(".") + 1);//加水印视频
